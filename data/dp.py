...
class Dataset():
    def __init__(self, input_res, output_res, num_parts, annot):
    ...
    def tf_dataset_from_generator(self, batch_size):
        return tf.data.Dataset.from_generator(
            ...
            output_shapes=(
                [self.input_res, self.input_res, 3],
                [self.output_res, self.output_res, self.num_parts]),
            args=()
    ...
    def loadImage(self, idx, eval=False):
        ...
        cropped = utils.img.crop(
            orig_img, c, s, (self.input_res, self.input_res))
        for i in range(np.shape(orig_keypoints)[1]):
            if orig_keypoints[0, i, 0] > 0:
                orig_keypoints[0, i, :2] = utils.img.transform(
        ...
        mat_mask = utils.img.get_transform(
            center, scale, (self.output_res, self.output_res), aug_rot)[:2]

        mat = utils.img.get_transform(
            center, scale, (self.input_res, self.input_res), aug_rot)[:2]
        inp = cv2.warpAffine(
            cropped, mat, (self.input_res, self.input_res)).astype(np.float32)/255
        keypoints[:, :, 0:2] = utils.img.kpt_affine(
            keypoints[:, :, 0:2], mat_mask)
