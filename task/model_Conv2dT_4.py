...
class HourGlass(model):
    ...
    def _generate_network(self, level):
        ...
        self.layers_dict['Conv2DTranspose'] = layers.Conv2DTranspose(256, (4, 4), padding='same', data_format='channels_last', strides=2)
        if level > 1:
        ...
        self.layers_dict['last'] = layers.Conv2D(256, (1, 1), use_bias=False)
        self.layers_dict['bn_last'] = layers.BatchNormalization()
    ...
    def _call(self, level, inp):
        ...
        out = tf.concat([up1, up2], axis=-1)
        out = self.layers_dict['bn'](out)
        out = self.layers_dict['last'](out)
        out = self.layers_dict['bn_last'](out)
        out = tf.nn.relu(out)
